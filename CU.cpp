...
void CU::OP6(mainMemory &memory, mainMemory *Register, const QString &R, const QString &X, const QString &Y) {
    ...
    int xIndex = hexToInt(X);
    int yIndex = hexToInt(Y);
    int rIndex = hexToInt(R);


    double n1 = binaryToDouble(Register->memory[xIndex].binary);
    double n2 = binaryToDouble(Register->memory[yIndex].binary);
    double res = n1 + n2;


    QString RES = doubleToBinary(res);


    memoryUnit::count = -1;


    Register->memory[rIndex].binary = RES;

    double resultAsDouble = binaryToDouble(RES);
    Register->memory[rIndex].hex = toHex(resultAsDouble);
}
